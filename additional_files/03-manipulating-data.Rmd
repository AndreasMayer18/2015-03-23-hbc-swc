...
```{r, results='show', purl=FALSE}
seq(1, 10, by=2)
seq(5, 10, length.out=3)       # equal breaks of length.out
seq(50, by=5, length.out=10)   # returns a vector of length.out
seq(1, 8, by=3)                # sequence stops to stay below upper limit
```
Another useful function in creating sequences is `rep`, which _rep_licates elements of vector (or list, but not covered here).
...
rep(1:4, each = 2, len = 4)    # only first 4 elements displayed
...
1. The function `nrow()` on a `data.frame` returns the number of rows. Use it,
   ...
   `data_by_2` that includes every other row of the metadata
